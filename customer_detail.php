...
(() => {
    ...
    function renderDetail() {
        ...
        const qrBlock = c.qr_code
            ? `<div class="mt-3"><img class="w-36 h-auto object-contain rounded-xl border border-gray-200 bg-white cursor-pointer previewable" src="${escapeHtml(c.qr_code)}" data-src="${escapeHtml(c.qr_code)}" data-title="客户二维码" alt="客户二维码"><div class="flex gap-2 mt-2"><button class="text-xs text-gray-500 bg-gray-100 rounded-lg px-3 py-1.5 hover:bg-gray-200 transition-colors" type="button" id="generateQrBtn">刷新二维码</button><a class="text-xs text-[#576B95] bg-blue-50 rounded-lg px-3 py-1.5 hover:bg-blue-100 transition-colors" href="${escapeHtml(c.user_url)}" target="_blank" rel="noopener">打开客户页</a></div></div>`
